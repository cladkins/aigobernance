/* AI Governance Kickstart survey — clone with client-side package download (no email send). */
(function () {
  'use strict';
    ...
    PERSONA.forEach(pq => {
      ...
      if (pq.type === 'single' && pq.options.some(o => o.help)) {
        ...
        pq.options.forEach(o => {
          ...
          card.addEventListener('click', () => { state.persona[pq.id] = o.v; applyDefaults(); renderPersona(); renderAll(); });
  ...
  function buildPolicyMarkdown(name, email) {
    ...
    md += '> Generated by AI Governance Kickstart, by BlueVoyant on ' + date + '.\n\n';
  ...
  function buildIntune(enforced) {
    return JSON.stringify({
      ...
      displayName: 'AI Governance Kickstart — Baseline',
  ...
  function buildJamf(enforced) {
    let x = '<?xml version="1.0" encoding="UTF-8"?>\n<!-- AI Governance Kickstart — Jamf configuration profile (skeleton) -->\n';
    x += '<!DOCTYPE plist PUBLIC "-//Apple//DTD PLIST 1.0//EN" "http://www.apple.com/DTDs/PropertyList-1.0.dtd">\n<plist version="1.0"><dict>\n';
    x += '  <key>PayloadDisplayName</key><string>AI Governance Kickstart Baseline</string>\n';
    x += '  <key>PayloadIdentifier</key><string>com.bluevoyant.aigovkickstart</string>\n';
  ...
  function buildBash(enforced) {
    let s = '#!/usr/bin/env bash\n# AI Governance Kickstart — enforcement (Bash skeleton)\n# Generated ' + new Date().toISOString() + '\n# ' + enforced.length + ' controls enforced. Review before running in production.\nset -euo pipefail\n\n';
  ...
  function buildExecSummary(name, email, a) {
    ...
    md += '_Board-ready overview · Generated by AI Governance Kickstart, by BlueVoyant · ' + date + '_\n\n';
    md += '**Prepared for:** ' + name + ' (' + email + ')\n\n';
    md += '---\n\n## Bottom line\n\n';
  ...
  async function generatePackage(name, email) {
    ...
    zip.file('README.txt',
      'AI Governance Kickstart — Governance Package, by BlueVoyant\n' +
      '==============================================\n\n' +
  ...
  function runAnalysis() { currentAnalysis = AIGK.scoring.analyze(state.persona, selectedControls()); return currentAnalysis; }
  ...
  async function handleDownload() {
    ...
    try {
      ...
      lastBlobUrl = url;
      const fname = 'AI-Governance-Kickstart-Package-' + new Date().toISOString().slice(0, 10) + '.zip';
